...
const FIREWORK_GENERATION_RATE = 0.02;
const HOLD_FIREWORK_GENERATION_RATE = 0.25;
...
function update() {
  ...
  // Create continuous fireworks if mouse is held down
  if (isMouseDown) {
    if (currentTime - mouseDownTime > HOLD_THRESHOLD) {
      shouldSpawnContinuously = true;
    }
    if (shouldSpawnContinuously) {
      if (Math.random() < HOLD_FIREWORK_GENERATION_RATE * deltaTime) {
...
canvas.addEventListener('mouseup', () => {
  isMouseDown = false;
  shouldSpawnContinuously = false; // Stop continuous spawning
});
